%% Pipeline to process the Planarian Data - Dark-Light-Dark Change
...
for p = 1:length(inpath)
    ...
    %% 2.3. Compute variance across time to identify empty channels
        ...
        % 2.3.3. Cut into light and dark cycles: each has 300 seconds, take
        % the middle 200
        % dark 1: 50 to 250s       
        cfg = [];
        cfg.latency = [50 250];

        data_psc{1} = ft_selectdata(cfg,data_ps); % data_psc 1 is the cut to the first dark phase
        
        % light 1: 350 to 550s
        cfg = [];
        cfg.latency = [350 550];

        data_psc{2} = ft_selectdata(cfg,data_ps); % data_psc 1 is the cut to the first dark phase
        
        % dark 2: 700 to 900s
        cfg = [];
        cfg.latency = [700 900];

        data_psc{3} = ft_selectdata(cfg,data_ps); % data_psc 1 is the cut to the first dark phase
        
    %% 2.4. Cut into smaller segments
    % Loop through the three segments
    for s = 1:length(data_psc)
        cfg = [];
        cfg.length = 6; % 10 seconds
        %cfg.overlap = .25; % 3.75 s overlap to get a continuous change in the spectrum
        data_psc{s} = ft_redefinetrial(cfg,data_psc{s});

        %% 2.5. Automatic artefact removal
        % 2.5.1 Get mean within trials for peak threshold
        for c = 1:length(data_psc{s}.label)
            for t = 1:length(data_psc{s}.trial)
               mean_tmp(c,t) = mean(data_psc{s}.trial{t}(c,:));
            end
        end
        ...
        data_pscr = vt_autoreject(cfg,data_psc{s});
        ...
        if  length(data_pscr.trial) > 5 & size(data_pscr.trial{1},1) > 0 
            ...
            % Save the data
            save([outpath{p},indat(v).name,'_',sprintf('%i',s),'.mat'],'data_expo','data_fft','-V7.3');
